...
@section('content')
    ...
    <div class="container mx-auto mt-8 mb-8" id="movies" data-scroll-index="1">
        ...
        <div class="flex justify-center items-center">
            <a href="{{ url('/movies/all') }}" class="btn btn-primary mt-6 text-white text-center rounded-full">Lihat Lebih Banyak</a>
        </div>
    </div>
@endsection
...
@push('js')
    ...
    <script>
        $(document).ready(function() {
            // Ketika tombol "Mulai Menjelajah" diklik
            $('a[href="#movies"]').on('click', function(event) {
                event.preventDefault(); // Mencegah pengalihan default ke href

                // Scroll ke bagian #movies-container dengan smooth
                $('html, body').animate({
                    scrollTop: $('#movies').offset().top
                }, 700); // Durasi animasi dalam milidetik (1000 = 1 detik)
            });
        });
    </script>
    <script>
        $(document).ready(function() {
            ...
            // Jika ada query di URL, lakukan pencarian otomatis saat halaman dimuat
            if (query) {
                $('#search-input').val(query);
                performSearch(query);
            } else {
                // Jika tidak ada query, ambil data film awal
                $('#loading').removeClass('hidden');
                $.ajax({
                    url: "{{ route('movie.search') }}", // Mendapatkan data film awal
                    method: "GET",
                    success: function(data) {
                        $('#loading').addClass('hidden');
                        // Menampilkan hasil data film di #movies-container
                        $('#movies-container').html(data); // Menampilkan hasil render HTML
                    }
                });
            }
            ...
            $('#search-input').on('input', function() {
                ...
                if (query === "") {
                    // Hapus parameter query dari URL jika kosong
                ...
                $('#loading').removeClass('hidden');
                $('#no-results').addClass('hidden'); // Menyembunyikan pesan tidak ditemukan

                // Panggil fungsi untuk melakukan pencarian
                performSearch(query);
            });
            ...
            function performSearch(query) {
                $.ajax({
                    ...
                    success: function(response) {
                        ...
                        // Jika hasil pencarian kosong, tampilkan pesan tidak ditemukan
                        if (response.trim() === '') {
                            $('#movies-container').html(''); // Hapus konten film
                            $('#no-results').removeClass('hidden'); // Tampilkan pesan "Tidak Ditemukan"
                        } else {
